# -*- coding: utf-8 -*-
"""
ADOFAI MIDI Converter - Python版本
将MIDI文件转换为A Dance of Fire and Ice的地图文件

支持两种模式：
1. RW模式 (pathData) - 使用 SetSpeed + Twirl
2. angleData模式 - 纯角度控制
...
from lib.midi.pathD import PathDataConverter
...
def convert_midi_to_adofai(midi_path: str, disable: list, octave_offset: int, 
                           mode: int, base_bpm: float = None) -> str:
    """转换MIDI文件到ADOFAI格式"""
    print()
    print(t('ui.separator'))
    ...
    # 生成输出路径
    mode_suffix = "_rw" if mode == 1 else "_angle"
    idx = midi_path.rfind('.')
    if idx != -1:
...
def main() -> None:
    ...
    try:
        ...
        # angleData 模式需要基准 BPM
        base_bpm = None
        if mode == 2:
            base_bpm = get_base_bpm()
        
        # 执行转换
        out_path = convert_midi_to_adofai(midi_path, disable, octave_offset, mode, base_bpm)
